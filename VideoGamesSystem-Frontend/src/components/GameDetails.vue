<template>
  ...
  import ReviewList from "../components/ReviewList.vue"; // Import the ReviewList component
  
  export default {
    name: "GameDetails",
    components: { ReviewList }, // Register the ReviewList component
    props: ["gameId"], // Receive gameId as a prop
    data() {
      return {
        game: null,
        store, // Add store to access userType and user details
      };
    },
    created() {
      this.fetchGame();
    ...
    methods: {
      async fetchGame() {
        const gameId = this.gameId; // Access the gameId passed as a prop
        try {
